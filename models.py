...
from superset.models.core import CustomParameters
...
class BaseDatasource(AuditMixinNullable, ImportMixin):
    ...
    def get_coloumns(self):
        # Load Colour Palette Template data and convet it to an array to pass it to the front end
        db_items = db.session.query(CustomParameters).all()
        db_items = sorted(db_items, key=lambda items: items.param_name, reverse=True)
        coloumn_list = []

        if self.name == 'custom_parameters':

            for pn in db_items:
                data_item = {}
                data_item['expression']=pn.param_name
                data_item['verbose_name']=pn.param_name
                data_item['description']=pn.param_name
                data_item['column_name']=pn.param_name
                data_item['filterable']=True
                coloumn_list.append(data_item)
        else:
            coloumn_list = [o.data for o in self.columns]
        return coloumn_list
    ...
    def explore_url(self):
        if self.default_endpoint:
            ...
            return '/qei/explore/{obj.type}/{obj.id}/'.format(obj=self)
